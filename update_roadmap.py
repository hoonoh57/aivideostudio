# D:\aivideostudio\update_roadmap.py
"""로드맵 업데이트 + 파일 정리 + 푸시"""
...
# 1) 루트의 정리 대상 파일 확인
print("=== 루트 디렉토리 스크립트 파일 ===")
scripts = []
for f in sorted(ROOT.iterdir()):
    if f.suffix == ".py" and f.name not in ("__init__.py", "update_roadmap.py"):
        if any(kw in f.name for kw in ["diag_", "fix_", "check_", "push_", "implement_"]):
            scripts.append(f.name)
            print(f"  {f.name}")
print(f"  Total: {len(scripts)} files to move to tools/")
...
for name in scripts:
    ...
    if src.exists():
        src.rename(dst)
        moved += 1
print(f"\n[MOVED] {moved} scripts → tools/")

# 3) Write updated roadmap
roadmap = r"""# AIVideoStudio (AVS) — 개발 로드맵 & TODO
...
print("\n=== Git ===")
run("git add -A")
run('git commit -m "v0.5.20: update ROADMAP.md, move diagnostic scripts to tools/"')
