# D:\aivideostudio\rewrite_both_files.py
"""Rewrite timeline_panel.py and export_panel.py completely with correct indentation"""
...
# ═══════════════════════════════════════════════════════════════
# 1. timeline_panel.py - COMPLETE REWRITE
# ═══════════════════════════════════════════════════════════════
tp = BASE / "aivideostudio" / "gui" / "panels" / "timeline_panel.py"

tp.write_text(r'''import math
from pathlib import Path
...
print(f"[SAVED] timeline_panel.py")

# ═══════════════════════════════════════════════════════════════
# 2. export_panel.py - COMPLETE REWRITE
# ═══════════════════════════════════════════════════════════════
ep = BASE / "aivideostudio" / "gui" / "panels" / "export_panel.py"

ep.write_text(r'''"""Export Panel — timeline-based export with FFmpeg + NVENC + audio mixing."""
import os
import re
import shutil
import subprocess
...
print(f"[SAVED] export_panel.py")

# ═══════════════════════════════════════════════════════════════
# 3. Ensure main_window.py has set_timeline_canvas connection
# ═══════════════════════════════════════════════════════════════
...
if "set_timeline_canvas" not in mw_src:
    ...
    if idx != -1:
        ...
        print("[OK] main_window.py: added set_timeline_canvas connection")
    else:
        print("[WARN] Could not find anchor in main_window.py")
else:
...
if all_ok:
    print("ALL OK! Run:")
    print("  python -m aivideostudio.main")
    print("\nThen:")
    print('  git add -A && git commit -m "v0.5.16: export In/Out range markers, zone bar, I/O keys" && git push')
else:
    print("ERRORS - check above")
print("=" * 60)
